...
| `inter_clique_edges` | string | Edge mode: "ring", "ring_extra" (default), "ring_star", "small_world", "fractal", "fully_connected" |
...
| `ring_star_extra` | int | Additional random edges per clique when using `ring_star` or `ring_extra` |
...
- `ring_extra` (default): ensure ring connectivity, then add `ring_star_extra` random inter-clique edges per clique to shorten paths without introducing a central hub.
- `ring_star`: ensure ring connectivity, then select the largest clique (ties → lowest index) as the central hub and connect it to every other clique, forming a star overlay on the ring.
- `ring_star_extra`: optional integer (`ring_star_extra`) adds that many additional pseudo-random edges per clique (beyond the base overlay) for either `ring_extra` or `ring_star`. This densifies the overlay so fan-out nodes have multiple options without relying on a hub.
